...
function is_real_email_address(?string $email): bool
{
    ...
    if (preg_match('/@(school\.local|example\.(com|org|net)|test|invalid)$/i', $email)) {
        return false;
    }
    return true;
}
...
function portal_welcome_subject(string $role): string
{
    $role = ucfirst(trim($role));
    return school_meta('system_name', 'LearnTrack Schools') . ' - Your ' . $role . ' Account';
}

function portal_welcome_html(array $payload): string
{
    $role = strtolower((string) ($payload['role'] ?? 'user'));
    $systemName = school_meta('system_name', 'LearnTrack Schools');
    $displayRole = ucfirst($role);
    $name = e((string) ($payload['name'] ?? 'User'));
    $email = e((string) ($payload['email'] ?? ''));
    $password = e((string) ($payload['password'] ?? ''));
    $loginUrl = e(portal_login_url());

    $extra = '';
    if ($role === 'student') {
        $studentCode = e((string) ($payload['student_code'] ?? ''));
        $className = e((string) ($payload['class_name'] ?? ''));
        $sectionName = e((string) ($payload['section_name'] ?? ''));
        $year = e((string) ($payload['year'] ?? ''));
        $extra .= '<p><strong>Student Code:</strong> ' . $studentCode . '</p>';
        if ($className !== '') {
            $extra .= '<p><strong>Class:</strong> ' . $className . '</p>';
        }
        if ($sectionName !== '') {
            $extra .= '<p><strong>Section:</strong> ' . $sectionName . '</p>';
        }
        if ($year !== '') {
            $extra .= '<p><strong>Academic Year:</strong> ' . $year . '</p>';
        }
    } elseif ($role === 'teacher' && trim((string) ($payload['designation'] ?? '')) !== '') {
        $extra .= '<p><strong>Designation:</strong> ' . e((string) $payload['designation']) . '</p>';
    }

    return '<div style="font-family:Arial,Helvetica,sans-serif;line-height:1.6;color:#222">'
        . '<h2 style="margin:0 0 12px">Welcome to ' . e($systemName) . '</h2>'
        . '<p>Hello ' . $name . ',</p>'
        . '<p>Your portal account has been created successfully.</p>'
        . '<p><strong>Role:</strong> ' . e($displayRole) . '</p>'
        . '<p><strong>Login Email:</strong> ' . $email . '</p>'
        . '<p><strong>Temporary Password:</strong> ' . $password . '</p>'
        . $extra
        . '<p><strong>Login Link:</strong> <a href="' . $loginUrl . '">' . $loginUrl . '</a></p>'
        . '<p>Please log in and change your password as soon as possible.</p>'
        . '<p>Regards,<br>' . e($systemName) . '</p>'
        . '</div>';
}

function notify_new_portal_user(array $payload): array
{
    ...
    if (!is_real_email_address($email)) {
        log_activity([
            ...
            'module_name' => $role . '_notifications',
            'record_id' => $recordId,
            'description' => 'Skipped welcome email for ' . $role . ' ' . $name . ' because the email address is missing or placeholder.',
            'new_values' => json_encode(['email' => $email, 'reason' => 'invalid_or_placeholder']),
        ]);
        return ['ok' => false, 'reason' => 'skipped_invalid_email'];
    }

    $subject = portal_welcome_subject($role);
    $html = portal_welcome_html($payload);
    $sent = send_portal_email($email, $subject, $html);

    log_activity([
        'action' => $sent ? 'email_sent' : 'email_failed',
        'module_name' => $role . '_notifications',
        'record_id' => $recordId,
        'description' => ($sent ? 'Sent' : 'Failed to send') . ' welcome email to new ' . $role . ' ' . $name . '.',
        'new_values' => json_encode(['email' => $email, 'subject' => $subject, 'sent' => $sent]),
    ]);

    return ['ok' => $sent, 'reason' => $sent ? 'sent' : 'failed'];
}
